...
以下のように定義を行います。  
こうすることで、このファイルで定義されたクラスは  
指定した名前空間の配下のクラスになります。
...
よく間違えやすいのが、「名前空間はファイルシステムの「フォルダ」のイメージに近い」と
説明しましたが、あくまでも考え方が近いだけで別物と考えてください。

例えば同じ`app`ディレクトリ内に「`A.php(Aクラス)`」、「`B.php(Bクラス)`」があった場合  
同じディレクトリであっても、2つは異なる名前空間を設定することが出来ます。

```php
// app/A.php  完全修飾名:\App\A
namespace App;
...
```php
// app/B.php  完全修飾名:\Test\B
namespace Test;

class B {}
```

ディレクトリ構成と名前空間は合わせるというルールのプロジェクトが多いため  
混同しやすいので注意しましょう。
